 ...
 *     IMPORTANT NOTE: When project a shapefile with sphere R to NAD83, 
 *                     there is no datum transformation here from PROJ4.6. 
 *                     Since WRF internal spatail data (USGS GLCC
 *                     , soil, and elevation) are in WGS84 datum (similar to NAD83), 
 *                     no datum transformation is needed from Sphere R to 
 *                     NAD83 ot WGS84 in order to match WRF internal spatial data.
 *                      
 ...
 *
 * Usage: toNLCDRaster.exe in_shapeFile out_rasterFile processedNLCDDir in_modisFile out_modisFile
 *        For instance:
...
int main( int nArgc,  char* papszArgv[] )
{
    ...
    const int   argsN = 5;  //number of input arguments required
    const char  *pszSrcFilename = NULL;
    const char  *polyID = NULL;
    const char  *pszDstFilename = NULL;    //domain shapefile
    const char  *pszwgsFilename = NULL;    //temp file -- domain shapefile projected in WRF R and treated as WGS84 datum
    const char  *psztmpFilename = NULL;    //temp file -- domain shapefile projected in NCLD projection
    char        *pszProj4 = NULL, *m_pszProj4=NULL;   //NLCD and MODIS projection
    char        *pszProj4_shp = NULL, *pszProj4_shp_geo = NULL;   //domain shapefile projections  
    char        *pszProj4_shp_wgs84 = "+proj=latlong +datum=WGS84";  //WRF or MM5 lat-long  projection 
    ...
    char       *pszWKT_nc = NULL;
    OGRSpatialReference oSRS, *oSRS_shp;
    string          rDataFile;
...
/* -------------------------------------------------------------------- */
    ...
    {
        imageStr = string(dirp->d_name);
        imageStr = trim(imageStr);
        if ( imageStr.find(".bil") !=string::npos )
        {
           i = imageStr.length() - 4;  //.bil position
           //printf ("lenght %s = %d\n", imageStr.c_str(),i);
           tmp_str = imageStr.substr(i,4);
           if ( tmp_str.compare(".bil" ) == 0 )
           {
    ...
    if ( imageStr.empty() )
    {
       printf ( "Error: no *.bil image file in dir: %s\n",dataDir.c_str() );
       exit ( 1 );
    }
...
/* -------------------------------------------------------------------- */
/*     Project grid shapefile into Lat/Long sphere                      */
/*     Redefine Lat/Long sphere as Lat/Long WGS84 as in WRF and MM5     */
/* -------------------------------------------------------------------- */

    //get projection of the domain shapefile
    printf( "\nObtain projection from the domain shapefile:  %s\n", pszSrcFilename );
    poDS = OGRSFDriverRegistrar::Open( pszSrcFilename, FALSE );
    if( poDS == NULL )
    {
      printf( "\tError: Open shapefile file failed: %s.\n", pszSrcFilename );
      exit( 1 );
    }
    ...
    oSRS_shp = poLayer->GetSpatialRef( );
    if (oSRS_shp == NULL)
    {
      printf("\tError: No projection information for file: %s\n", pszSrcFilename);
    }

    oSRS_shp->exportToProj4( &pszProj4_shp );
    printf ( "\tProj4 from the domain shapefile = %s\n", pszProj4_shp);

    //get lat and long projection 
    pszProj4_shp_geo = convert2LatLongProjection (pszProj4_shp);
    printf ( "\tProj4 from the domain shapefile in lat-long:  %s\n", pszProj4_shp_geo);
    ...
    cmd_str.append(pszProj4_shp_geo);
    cmd_str.append("\"  ");

    //create shapefile name in WRF R projection
    ...
    string tmp_nlcd_file_str = tmp_str;         //make a string copy for file name in NLCD projection
    tmp_nlcd_file_str.append ( "_nlcd.shp" );   //file name string for NLCD projection to be used in the next projecting
    tmp_str.append( "_wrf_r.shp" );             //file name string for WRF R projection

    pszwgsFilename = tmp_str.c_str();  //temp shapefile name
    //printf (" TEMP WRF R File = %s\n",pszwgsFilename);

    //if the file does exist delete it 
    if (stat(pszwgsFilename, &stFileInfo) == 0) 
    {
       printf( "\tTemp WRF R shapefile exists and delete it: %s\n", pszwgsFilename );
       ...
       if ( (poDriver->DeleteDataSource( pszwgsFilename )) != OGRERR_NONE )
       {
          printf( "\tError in deleting temp shapefile: %s\n\n", pszwgsFilename );
       }
    }

    printf("\nProjecting %s to WRF R projection and stored in: %s\n",pszSrcFilename, pszwgsFilename);
 
    cmd_str.append(" ");
    cmd_str.append(pszwgsFilename);
    cmd_str.append(" ");
    ...
    printf("\tSuccessful in projecting the shapefile to WRF R lat and long projection.\n\n");

/* -------------------------------------------------------------------- */
/*     Project shapefile from WGS84 into NLCD NAD83 projection                */
/* -------------------------------------------------------------------- */

    //create shapefile name in NLCD projection
    psztmpFilename = tmp_nlcd_file_str.c_str();  //NLCD temp shapefile name
    //printf (" TEMP NLCD File = %s\n",psztmpFilename);

    printf("\nProjecting %s to NLCD projection and stored in: %s\n",pszwgsFilename, psztmpFilename);

    //if the file does exist delete it
    if (stat(psztmpFilename, &stFileInfo) == 0)
    {
       printf( "\tTemp projected NLCD shapefile exists and delete it: %s\n", psztmpFilename );
       poDriver = OGRSFDriverRegistrar::GetRegistrar()->GetDriverByName( pszDriverName );
       if( poDriver == NULL )
       {
          printf( "%s driver not available.\n", pszDriverName );
    ...
    //build command line to call ogr2ogr
    cmd_str = string( gdalBinDir );
    cmd_str.append("ogr2ogr -s_srs \"");
    cmd_str.append(pszProj4_shp_wgs84);    //WRF R lat and long are treated as WGS84 lat and long
    ...
    //delete the wgs temp shapefile
    if (stat(pszwgsFilename, &stFileInfo) == 0)
    {
       printf( "\tDelete temp WRF R shapefile: %s\n", pszwgsFilename );
       poDriver = OGRSFDriverRegistrar::GetRegistrar()->GetDriverByName( pszDriverName );
       if( poDriver == NULL )
       {
          printf( "%s driver not available.\n", pszDriverName );
    ...
    printf("\tSuccessful in projecting WRF WGS84 shapefile to NLCD format.\n\n");
...
/* -------------------------------------------------------------------- */
    ...
    printf("Raterizing projected the shapefile to NLCD 30m grids...\n");
...
/* -------------------------------------------------------------------- */
    ...
    printf( "\tDelete temp NLCD shapefile: %s\n", psztmpFilename );
    poDriver = OGRSFDriverRegistrar::GetRegistrar()->GetDriverByName( pszDriverName );
    if( poDriver == NULL )
    {
       printf( "%s driver not available.\n", pszDriverName );
...

{
    ...
    printf( "Usage: poly2NLCDRaster.exe  \"PROJ4 NLCD Projection_definition\"  in_shapeFile  out_rasterFile\n");
    exit( 1 );
}
