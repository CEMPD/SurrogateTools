...
/* Forward */
static int occompile1(OCconnection* state, OCnode* node, OCmemdata** memdatap, XDR*);
static int occompilefields(OCconnection* state, OCnode* node, OCmemdata** memdatap, XDR*);
static int occompileprim(OCconnection* state, OCnode* node, OCmemdata** memdatap,XDR*);
...
oc_compile(OCconnection* state)
{
    ...
    ocstat = occompile1(state,state->schema.datadds.tree,&memdata,xdrs);
    if(ocstat == OC_NOERR) {
	state->content.memdata = memdata;
    }
    return ocstat;
}
...
static int
occompile1(OCconnection* state, OCnode* node, OCmemdata** memdatap, XDR* xdrs)
{
    ...
    nelements = totaldimsize(node);
    nfields = oclistlength(node->subnodes);
	...
	if(node->array.rank == 0) {
	    ocstat = occompilefields(state,node,&memdata,xdrs);
	    if(ocstat != OC_NOERR) goto fail;
	} else { /* rank > 0 */
	    memdata = makememdata(node->octype,OC_NAT,nelements);
		...
		ocstat = occompilefields(state,node,&structdata,xdrs);
		if(ocstat != OC_NOERR) goto fail;
	        pmem[i] = structdata;
	        structdata = NULL;
	    }
    ...
    case OC_Primitive:
	ocstat = occompileprim(state,node,&memdata,xdrs);
	if(ocstat != OC_NOERR) goto fail;
	break;
...
static int
occompilefields(OCconnection* state, OCnode* node, OCmemdata** memdatap, XDR* xdrs)
{
    OCmemdata* structdata = NULL;
    OCmemdata** qmem;
    OCerror ocstat = OC_NOERR;
	...
	if(ocstat != OC_NOERR) {freeocmemdata(structdata); return ocstat;}
	qmem[i] = fielddata;		
    }
...
static int
occompileprim(OCconnection* state, OCnode* node, OCmemdata** memdatap, XDR* xdrs)
{
    ...
    OCASSERT((node->octype == OC_Primitive));
    ...
    memdata = makememdata(node->octype,node->etype,nelements);
    MEMCHECK(memdata,OC_ENOMEM);
    memdata->mode = (node->array.rank > 0?Dimmode:Datamode);
    ...
    case OC_UByte:
    case OC_Char: {
        if(node->array.rank == 0) { /* Single unpacked character/byte */
    ...
    case OC_Int16: case OC_UInt16: {
        ...
        for(i=0;i<xdrcount;i++) { /* Write in place */
            unsigned int hostint = ntohl(src[i]);
            dst[i] = (unsigned short)hostint;                               
        }
        ocfree(src);
    } break;
    ...
    case OC_Float32: {
        ...
        for(i=0;i<xdrcount;i++) {dst[i] = ntohl(dst[i]);}
    } break;
    ...
    case OC_Float64: {
        ...
        if(!xdr_opaque(xdrs,(char*)dst,xdrsize)) {ocstat = OC_EXDR; goto fail;}
        for(i=0;i<2*xdrcount;i++) {dst[i] = ntohl(dst[i]);}
    ...
    default: OCPANIC1("unexpected etype: %d",node->etype);
    } /* switch */
...
#ifdef IGNORE
static int
mark(OCbytes* buf, char** memp, unsigned int memsize)
{
    unsigned int len0 = ocblength(buf);
    if(!ocbsetlength(buf,len0+(2*memsize)) /* 2X => Keep some headroom */
	return OC_ENOMEM;
    *memp = ocbcontents(buf)
    *memp += len0;
    return OC_NOERR;    
}
static void
alignbuffer(OCtype octype, OCbytes* buf)
{
    unsigned int buflen = ocblength(buf);
    buflen += ocpad(octype,buflen);
    ocbsetlength(buf,buflen);
}
#endif
