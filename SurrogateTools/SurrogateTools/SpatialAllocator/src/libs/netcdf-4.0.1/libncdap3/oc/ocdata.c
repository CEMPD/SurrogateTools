...
/* Skip arbitrary dimensioned instance*/
/* Handles dimensioning.*/
int
oc_skip(OCnode* node, XDR* xdrs)
{
    ...
    switch (node->octype) {
        case OC_Primitive:
        	...
        	OCASSERT((node->xdrdimsize > 0 && node->xdrinstancesize > 0));
		if(node->array.rank > 0) {
        	    if(!xdr_skip(xdrs,node->xdrdimsize)) return xdrerror();
		} else {
        	    if(!xdr_skip(xdrs,node->xdrinstancesize)) return xdrerror();
		}
            }
	    break;
	    ...
	    OCASSERT((node->array.rank == 0));
            if(node->xdrinstancesize > 0) { /* do a direct skip*/
                if(!xdr_skip(xdrs,node->xdrdimsize)) return xdrerror();
		break;
...
int
oc_skipinstance(OCnode* node, XDR* xdrs)
{
    unsigned int i;
    int stat = OC_NOERR;
    unsigned int xdrcount;
	...
	case OC_Primitive:
		...
		if(!xdr_skip(xdrs,xdrcount)) return xdrerror();
	        ...
	        OCASSERT((node->xdrinstancesize > 0));
                if(!xdr_skip(xdrs,node->xdrinstancesize)) return xdrerror();
	    }
	    break;
...
*/
int
oc_xdrread(XDR* xdrs, char* memory, size_t memsize, int packed, OCtype octype, unsigned int start, size_t count)
{
    int stat = OC_NOERR;
    unsigned int i;
	...
	for(p=(unsigned int*)startmem,i=0;i<2*count;i++,p++) {*p = ntohl(*p);}
    } else {
	unsigned int* p;
	...
	if(!xdr_opaque(xdrs,(char*)localmem,xdrsize)) return xdrerror();
	for(p=(unsigned int*)startmem,i=0;i<count;i++,p++) {*p = ntohl(*p);}
    }
