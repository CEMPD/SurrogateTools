  ...
  function nf90_create(path, cmode, ncid, initialsize, chunksize, cache_size, &
    ...
    ! Is user trying to use netCDF-3 stuff on a netCDF-4 file?
    if((present(initialsize) .or. present(chunksize)) .and. &
         btest(cmode, NETCDF4_BIT)) then
       nf90_create = nf90_enotnc3
       return
    end if

    ! Is user trying to use netCDF-4 stuff on a netCDF-3 file?
    if((present(cache_size) .or. present(cache_nelems) &
         .or. present(cache_preemption) .or. present(comm) &
         .or. present(info)) .and. &
         .not. btest(cmode, NETCDF4_BIT)) then
       nf90_create = nf90_enotnc4
       return
    end if
