...
dumpdataprim(Constant* ci, Bytebuffer* buf)
{
	...
	bbCat(buf,"_");
	break;
    default: FPANIC("dumpdataprim: bad type code:%d",ci->nctype);
    }
}
...
dumpsrc0(Datasrc* src,char* tag)
{
    ...
    if(src->autopop) fprintf(stderr,"*");
    do {
        fprintf(stderr,"[%d/%d]",src->index,src->length);
	for(i=0;i<index;i++) {
	    fprintf(stderr," ");
	    dumpconstant(&src->data[i]);
	}
	fprintf(stderr,"^");
    ...
    if(con != NULL && con->nctype == NC_COMPOUND && con->value.compoundv != NULL) {
	Datalist* dl = con->value.compoundv;
	char* vname = (dl->schema?dl->schema->name:"null");
	Odometer* odom = dl->dimdata;
	if(odom != NULL) {
	    fprintf(stderr,";    odometer %s = %s",vname,odometerprint(odom));
	}
    }
    fprintf(stderr,"\n");
    fflush(stderr);
