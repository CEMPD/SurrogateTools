...
case CASE(NC_OPAQUE,NC_OPAQUE):
    ...
    if(bytes != NULL) free(bytes); /* cleanup*/
...
setprimlength(Constant* prim, unsigned long len)
{
    ...
    if(prim->nctype == NC_STRING) {
	    ...
	    free(prim->value.stringv.stringv);
	    ...
	    char* s;
	    if((len % 2) != 0) len++;
	    diff = (len - prim->value.opaquev.len);
	    s = (char*)emalloc(len+1);
	    strcpy(s,prim->value.opaquev.stringv);
	    memset(s+len,'0',diff);
	    s[len] = '\0';
	    free(prim->value.opaquev.stringv);
