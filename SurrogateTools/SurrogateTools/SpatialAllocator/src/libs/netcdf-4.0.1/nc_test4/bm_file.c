/*
  ...
  $Id: bm_file.c,v 1.59 2009/02/17 19:48:05 ed Exp $
*/
#include <config.h>
#include <stdio.h>
#include <string.h>
...
usage(void)
{
   fprintf(stderr, "bm_file -v [-s N]|[-t V:S:S:S -u V:C:C:C -r V:I:I:I] -o file_out -f N -h"
	   " -c V:C:C,V:C:C:C -d -m -p -i -e 1|2 file\n%s", USAGE);
}
...
main(int argc, char **argv)
{
   ...
   while ((c = getopt(argc, argv, "vo:f:hc:dpms:it:u:r:e:")) != EOF)
      switch(c) {
	 case 'v':
	    verbose++;
	    break;
   ...
   if (!my_rank)
   {
	 ...
	 printf("deflate, shuffle, chunksize[0], chunksize[1], chunksize[2], "
      ...
      printf("%d, %d, %d, %d, %d, %d, %d, %d, %d, ", in_format, out_format, (int)file_size(file_in), 
	     (int)file_size(file_out), tmeta_read_us, tmeta_write_us, tdata_read_us, tdata_write_us,
	     endianness);
      if (doublecheck)
