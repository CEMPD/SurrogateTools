 ...
 *   $Header: /upc/share/CVS/netcdf-3/ncdump/ncdump.c,v 1.129 2009/03/19 15:20:23 russ Exp $
 *********************************************************************/
...
#define USAGE   "\
  ...
  file             Name of netCDF file\n"
    ...
    (void) fprintf(stderr,
		   "%s [-c|-h] [-v ...] [[-b|-f] [c|f]] [-l len] [-n name] [-p n[,n]] [-x] [-k] [-s] [-t] file\n%s",
		   progname,
		   USAGE);
...
#define FILE_DELIMITER '/'
    ...
    {
        extern char* nc__testurl(const char*);
 	char* base = nc__testurl(path);
	if(base != NULL) return base; /* Looks like a url */
    ...
    )
{
	...
	if(contig == 1) {
	   ...
	   int i;
		...
		printf("%d%s", chunkp[i], i+1 < varp->ndims ? ", " : " ;\n");
	    }
	    free(chunkp);
	}
    }
...
do_ncdump_rec(int ncid, const char *path, fspec_t* specp)
{
   ...
   if (specp->nlvars > 0) {
	   ...
	   * group syntax like "grp1/grp2/var" yet, so call intermediate
...
/* 	 printf ("%s%s", dim_name, id < var.ndims-1 ? ", " : ")"); */
	 ...
	 printf ("%s", id < var.ndims-1 ? ", " : ")");
	  ...
	  pr_att_specials(ncid, varid, &var);
      }
      free(var.dims);
   }
#else /* not using netCDF-4 */
