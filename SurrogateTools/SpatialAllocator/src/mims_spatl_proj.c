...
PJ *mimsSetProjection (MapProjInfo *map)
{
  ...
  /*Define Ellipsoid */  
    ...
    else if (map->ctype == LAMGRD3)
    {
      ...
      sprintf(args[i++],"%s=%lf","+x_0",map->p_gam);
      sprintf(args[i++],"%s=%lf","+y_0",map->xcent);
      sprintf(args[i++],"%s=%s","+units","m");
    }
    else if (map->ctype == AEAGRD3)
    {
      ...
      sprintf(args[i++],"%s=%lf","+x_0",map->p_bet);
      sprintf(args[i++],"%s=%lf","+y_0",map->p_gam); 
    }
    else if (map->ctype == MER1GRD3) {
      sprintf(mesg,"setting Mercator 1SP parameters");
      MESG(mesg);
      sprintf(args[0],"%s","+proj=merc");
      sprintf(args[i++],"%s=%lf","+lon_0",map->p_alp);
      sprintf(args[i++],"%s=%lf","+k_0",map->p_bet);
      sprintf(args[i++],"%s=%lf","+x_0",map->p_gam);
      sprintf(args[i++],"%s=%lf","+y_0",map->xcent);
    }
    else if (map->ctype == MER2GRD3) {
      sprintf(mesg,"setting Mercator 2SP parameters");
      MESG(mesg);
      sprintf(args[0],"%s","+proj=merc");
   ...
   if(map->custom_proj_str != NULL)
    { 
     ...
     for (i=0;i<argc;i++)
     {
		...
		if (strcmp(a_name, "+proj") == 0)
		{
		  map->ctype = LAEAGRD3;  /*define a contant*/
		}
		else if (strcmp(a_name, "+lat_0") == 0)
                {
                  map->p_alp = atof(a_value);
	    ...
	    if (strstr(map->custom_proj_str, "merc") != NULL && strstr(map->custom_proj_str, "k_0") != NULL)
            {
                if (strcmp(a_name, "+proj") == 0)
		{
		   map->ctype = MER1GRD3;
            ...
            if (strstr(map->custom_proj_str, "merc") != NULL && strstr(map->custom_proj_str, "lat_ts") != NULL)
            {
                if (strcmp(a_name, "+proj") == 0)
                {
                   map->ctype = MER2GRD3;
