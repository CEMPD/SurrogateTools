...
char *prog_version = "MIMS Spatial Allocator Version 3.4, 1/09/2008\n";
...
int main(int argc, char *argv[])
{
    ...
    else if(type == ALLOCATE)
    {
        ...
        if(!strcmp(tempString, "RegularGrid") || !strcmp(tempString, "Regulargrid") ||
           ...
           !strcmp(tempString, "EGrid")       || !strcmp(tempString, "Egrid"))
        {
        ...
        {
             dataMapProj =
                 getFullMapProjection(ENVT_OUTPUT_POLY_ELLIPSOID,
                                 ENVT_OUTPUT_POLY_MAP_PRJN);
            outputMapProj =
                  getFullMapProjection(ENVT_OUTPUT_ELLIPSOID, ENVT_OUTPUT_MAP_PROJ);

            p_data =
                   PolyReader(ENVT_OUTPUT_POLY_FILE, ENVT_OUTPUT_POLY_TYPE,
        ...
        MESG("\nFinished interseting input and output files......\n");

        if(outputIoapi)
        {
#ifdef USE_IOAPI

            MESG("\nWritting ioapi output file...\n");
