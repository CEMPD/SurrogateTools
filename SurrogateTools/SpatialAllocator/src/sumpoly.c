...
static int debugcty = 36061;    /* set this to a county of interest */
    ...
    *psum = sum = (double *) malloc(n1 * sizeof(double));
    ...
    for(i = 0; i < n; i++)
    {
        ...
        if(id >= 0)
        {
            ...
            /*fprintf(stderr,"processing county %s, id=%d\n",countyid, id); */
            ...
            if(use_weight_attr_value)
            {
                ...
                else /* it's a poly */ if (val != 0.0)
                {
                    ...
                    {
                        fprintf(stderr,
                                "sum1poly1: %d shape area = %lf, parent area = %lf, fract=%.4lf\n",
            ...
            else
            {
                ...
                else            /* polys */
                {
                    ...
                    {
                        fprintf(stderr,
                                "sum1poly2: %d shape area = %lf, frac=%lf\n",
        ...
        if(data_poly_idx >= 0 && grid_cell_idx >= 0)
        {
            if(use_weight_attr_value)
            {
                ...
                else
                {
                    ...
                    frac = val * (PolyArea(ps) / PolyArea(pp->p1->ps));
#ifdef DEBUGCOUNTY
                    countyid = d_poly->attr_val[data_poly_idx][0].ival;
                    fprintf(stderr, "sum2poly ctyid = %d\n", countyid);
