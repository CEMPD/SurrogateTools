...
int dods_compress = 0;
int dods_verify = 0;
struct Proxy *pstructProxy = NULL;
char *cook = NULL;
...
{
	if (!(name && pw)) {
		oclog(LOGERR, "Both username and password must be given.");
		return OC_EIO;
	}
...
read_dodsrc(char *in_file_name)
{
	...
	if (in_file == NULL) {
		oclog(LOGERR, "Could not open the .dodsrc file");
		return OC_EIO;
	}
		...
		if (more[0] != '#') {
			...
			*p = '\0';
			...
			if (strcmp(more, "USE_CACHE") == 0) {
				oclog(LOGWARN,"USE_CACHE: not currently supported");
			} else if (strcmp(more, "MAX_CACHE_SIZE") == 0) {
				oclog(LOGWARN,"MAX_CACHE_SIZE: not currently supported");
			} else if (strcmp(more, "MAX_CACHED_OBJ") == 0) {
				oclog(LOGWARN,"MAX_CACHED_OBJ: not currently supported");
			} else if (strcmp(more, "IGNORE_EXPIRES") == 0) {
				oclog(LOGWARN,"IGNORE_EXPIRES: not currently supported");
			} else if (strcmp(more, "CACHE_ROOT") == 0) {
				oclog(LOGWARN,"CACHE_ROOT: not currently supported");
			} else if (strcmp(more, "DEFAULT_EXPIRES") == 0) {
				oclog(LOGWARN,"DEFAULT_EXPIRES: not currently supported");
			} else if (strcmp(more, "ALWAYS_VALIDATE") == 0) {
				oclog(LOGWARN,"ALWAYS_VALIDATE: not currently supported");
			} else if (strcmp(more, "DEFLATE") == 0) {
				/* int v_len = strlen(v); unused */
				dods_compress = atoi(v);
				if (ocdebug > 1)
					oclog(LOGNOTE,"Compression: %d", dods_compress);
			} else if (strcmp(more, "VALIDATE_SSL") == 0) {
				dods_verify = atoi(v);
				if (ocdebug > 1)
					oclog(LOGNOTE,"SSL Verification: %d", dods_verify);
			} else if (strcmp(more, "PROXY_SERVER") == 0) {
				char *host_pos = NULL;
				char *port_pos = NULL;
				/* int v_len = strlen(v); unused */

				pstructProxy = malloc(sizeof(struct Proxy));
				if (!pstructProxy)
					return OC_ENOMEM;
...
#endif
				if (ocdebug > 1) {
					oclog(LOGNOTE,"host name: %s", pstructProxy->host);
					oclog(LOGNOTE,"user name: %s", pstructProxy->user);
					oclog(LOGNOTE,"password name: %s", pstructProxy->password);
					oclog(LOGNOTE,"port number: %d", pstructProxy->port);
				}

			} else if (strcmp(more, "NO_PROXY_FOR") == 0) {
				oclog(LOGWARN,"NO_PROXY_FOR: not currently supported");
			} else if (strcmp(more, "AIS_DATABASE") == 0) {
				oclog(LOGWARN,"AIS_DATABASE: not currently supported");
				...
				cook[v_len] = '\0';
				if (ocdebug > 1)
					oclog(LOGNOTE,"Cookie jar name: %s", cook);
			}
...
write_dodsrc(char *out_file_name)
{
    ...
    if (out_file == NULL) {
    	oclog(LOGERR,"cannot open output file\n");
    	return OC_EIO;
    }
