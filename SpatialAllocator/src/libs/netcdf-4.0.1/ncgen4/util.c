 ...
 *   $Header: /upc/share/CVS/netcdf-3/ncgen4/util.c,v 1.6 2009/03/11 18:26:19 dmh Exp $
 *********************************************************************/
...
/* 1. assume 3rd arg comes from strlen() (i.e. does not count trailing null)*/
/* 2. guarantee null termination.*/
void
strcopyn(char* dst, char* src, unsigned int len)
{
    strncpy(dst,src,len);
    dst[len] = '\0';
}
...
freeSymbol(Symbol* sym)
{
	...
	if(vsym->var.dims != NULL) free(vsym->var.dims);
	break;
    case NG_ATT:
...
hasunlimited(Dimset* dimset)
{
	...
	if(dim->dim.size == NC_UNLIMITED) return 1;
    }
    return 0;
}
...
{
   return (dimset->ndims > 0 && dimset->dimsyms[0]->dim.size == NC_UNLIMITED);
}
...
classicunlimited(Dimset* dimset)
{
    ...
    for(i=0;i<dimset->ndims;i++) {
	Symbol* dim = dimset->dimsyms[i];
	if(dim->dim.size == NC_UNLIMITED) last = i;
    }
    return (last < 1);
}
...
isbounded(Dimset* dimset)
{
    ...
    for(i=0;i<dimset->ndims;i++) {
	Symbol* dim = dimset->dimsyms[i];
	if(dim->dim.size == NC_UNLIMITED) return 0;
    }
    return 1;
}
...
poolalloc(size_t length)
{
    ...
    if(pool[poolindex] != NULL) free(pool[poolindex]);
    pool[poolindex] = (char*)malloc(length);
...
{
    return subarraylength(dimset,0,-1);
}

unsigned long
subarraylength(Dimset* dimset, int first, int last)
{
    /* compute the total n-dimensional size as 1 long array*/
    unsigned long totalsize = 1;
    int i;
    if(last == -1) last = dimset->ndims;
    for(i=first;i<last;i++) {
	if(dimset->dimsyms[i]->dim.size == NC_UNLIMITED) continue;
	totalsize = totalsize * dimset->dimsyms[i]->dim.size;
    }
    return totalsize;    
}
